...
<article class="course-card group">
    <a href="{{ route('courses.show', $course->slug) }}" class="block">
        ...
        <div class="thumbnail relative overflow-hidden rounded-t-md" style="aspect-ratio: 16/9;">
            ...
            <!-- Price Badge -->
            @if(!$course->isFree())
                <div class="absolute bottom-0 right-0 bg-black text-white px-6 py-2 rounded-tl-2xl">
                    <div class="flex flex-col items-end">
                        @if($course->sale_price && $course->sale_price < $course->price)
                            <!-- Có giá sale -->
                            <span class="text-xs line-through text-gray-400 heading-font">{{ number_format($course->price) }}đ</span>
                            <span class="text-base md:text-lg font-bold heading-font">{{ number_format($course->sale_price) }}đ</span>
                        @else
                            <!-- Chỉ có giá gốc -->
                            <span class="text-base md:text-lg font-bold heading-font">{{ number_format($course->price) }}đ</span>
                        @endif
                    </div>
                </div>
            @else
                <!-- Free Badge -->
                <div class="absolute bottom-0 right-0 bg-green-500 text-white text-[14px] md:text-[16px] heading-font px-6 py-2 rounded-tl-2xl uppercase">
                    Miễn phí
                </div>
            @endif
        </div>
